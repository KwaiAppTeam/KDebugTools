// Copyright 2021 Kwai, Inc. All rights reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//  http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

import 'dart:async';

import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:k_debug_tools_web/main.dart';
import 'package:k_debug_tools_web/src/app/model.dart';
import 'package:k_debug_tools_web/src/app_window_bloc.dart';
import 'package:k_debug_tools_web/src/bloc_provider.dart';
import 'package:k_debug_tools_web/src/event/PinEvent.dart';
import 'package:k_debug_tools_web/src/event_bus.dart';
import 'package:k_debug_tools_web/src/widgets/common_widgets.dart';
import 'package:k_debug_tools_web/src/widgets/root_navi_bar.dart';

import 'app_info_bloc.dart';

class AppInfoWindow extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      child: AppInfoWidget(),
      blocs: [AppInfoBloc(context)],
    );
  }
}

class AppInfoWidget extends StatefulWidget {
  @override
  _AppInfoWidgetState createState() => _AppInfoWidgetState();
}

class _AppInfoWidgetState extends State<AppInfoWidget> {
  AppInfoBloc _appInfoBloc;
  AppWindowBloc _windowBloc;

  @override
  void initState() {
    _appInfoBloc = BlocProvider.of<AppInfoBloc>(context).first;
    _windowBloc = BlocProvider.of<AppWindowBloc>(context).first;
    _appInfoBloc.initData().then((v) {
      setState(() {});
    });
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    return Container(
      color: Theme.of(context).colorScheme.background,
      child: _buildListWidget(),
    );
  }

  ///列表内容
  Widget _buildListWidget() {
    List<BaseKeyValue> base = <BaseKeyValue>[];
    base.add(BaseKeyValue('appName', _appInfoBloc.data['appName']));
    base.add(BaseKeyValue('package', _appInfoBloc.data['package']));
    base.add(BaseKeyValue('version', _appInfoBloc.data['version']));
    base.add(BaseKeyValue('buildNumber', _appInfoBloc.data['buildNumber']));

    List<BaseKeyValue> extra = <BaseKeyValue>[];
    (_appInfoBloc.data['extra'] as List)?.forEach((element) {
      extra.add(BaseKeyValue.fromMap(element));
    });
    ThemeData theme = Theme.of(context);
    return CustomScrollView(
      physics: AlwaysScrollableScrollPhysics(),
      slivers: <Widget>[
        SliverList(
            delegate: ListView.builder(
                itemCount: base.length,
                itemBuilder: (ctx, index) {
                  return _buildListWidgetItem(
                      theme, index, base.elementAt(index));
                }).childrenDelegate),
        SliverList(
            delegate: ListView.builder(
                itemCount: extra.length,
                itemBuilder: (ctx, index) {
                  return _buildListWidgetItem(
                      theme, index, extra.elementAt(index));
                }).childrenDelegate)
      ],
    );
  }

  Widget _buildListWidgetItem(ThemeData theme, int index, BaseKeyValue kv) {
    return Container(
      constraints: BoxConstraints(minHeight: 30),
      child: GestureDetector(
        behavior: HitTestBehavior.opaque,
        child: ListRow(
          separatorBuilder: (ctx, i) {
            return Container(width: 1, height: 30, color: theme.focusColor);
          },
          childPadding: EdgeInsets.only(left: 8, right: 8),
          children: <Widget>[
            Icon(
              Icons.label,
              size: 16,
            ),
            Expanded(
              flex: 2,
              child: Container(
                child: Text(
                  kv.key ?? '',
                  style: theme.textTheme.bodyText2,
                ),
              ),
            ),
            Expanded(
              flex: 5,
              child: Container(
                child: SelectableText(
                  kv.value ?? '',
                  style: theme.textTheme.bodyText2,
                ),
              ),
            ),
          ],
        ),
      ),
    );
  }
}

class AppInfoQuickItem extends StatefulWidget {
  @override
  _AppInfoQuickItemState createState() => _AppInfoQuickItemState();
}

class _AppInfoQuickItemState extends State<AppInfoQuickItem> {
  AppInfoBloc _appInfoBloc;

  StreamSubscription _pinStat;

  @override
  void initState() {
    _appInfoBloc = AppInfoBloc(context);
    _appInfoBloc.initData().then((value) => setState(() {}));
    //init after PinVerified
    _pinStat = eventBus.on<PinVerified>().listen((event) {
      _appInfoBloc.initData().then((value) => setState(() {
            analytics.setUserProperty(
                name: 'AppName', value: _appInfoBloc.data['appName']);
            analytics.setUserProperty(
                name: 'AppPackage', value: _appInfoBloc.data['package']);
            analytics.setUserProperty(
                name: 'AppVersion', value: _appInfoBloc.data['version']);
            analytics.setUserProperty(
                name: 'AppBuildNumber',
                value: _appInfoBloc.data['buildNumber']);
          }));
    });
    super.initState();
  }

  @override
  void dispose() {
    _pinStat?.cancel();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return Container(
      alignment: Alignment.center,
      height: ROOT_NAVI_HEIGHT,
      width: ROOT_NAVI_HEIGHT,
      child: Icon(
        Icons.info_outline,
        size: 33,
        color: Colors.white,
      ),
    );
  }
}
